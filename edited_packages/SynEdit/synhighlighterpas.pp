...
type
  ...
  TtckSubCommentKind = (tsckNone, tsckNote, tsckDivider, tsckTODO, tsckDone, tsckWarning);
  {/NEW}
...
type
 ...
 TSynPasRangeInfo = record
  ...
  private
    ...
    FSubCommentID: TtckSubCommentKind;
    {/NEW}
    ...
    fCommentAttri_Sub_Note: TSynHighlighterAttributes;
    fCommentAttri_Sub_Divider: TSynHighlighterAttributes;
    fCommentAttri_Sub_TODO: TSynHighlighterAttributes;
    fCommentAttri_Sub_DONE: TSynHighlighterAttributes;
    fCommentAttri_Sub_Warning: TSynHighlighterAttributes;
    {/NEW}
...
begin
  ...
  fCommentAttri_Sub_Note := TSynHighlighterAttributes.Create(SYNS_AttrComment+'_Note', SYNS_XML_AttrComment+'_Note');
  AddAttribute(fCommentAttri_Sub_Note);
  fCommentAttri_Sub_Divider := TSynHighlighterAttributes.Create(SYNS_AttrComment+'_Divider', SYNS_XML_AttrComment+'_Divider');
  AddAttribute(fCommentAttri_Sub_Divider);
  fCommentAttri_Sub_TODO := TSynHighlighterAttributes.Create(SYNS_AttrComment+'_TODO', SYNS_XML_AttrComment+'_TODO');
  AddAttribute(fCommentAttri_Sub_TODO);
  fCommentAttri_Sub_DONE := TSynHighlighterAttributes.Create(SYNS_AttrComment+'_DONE(Todo)', SYNS_XML_AttrComment+'_DONE(Todo)');
  AddAttribute(fCommentAttri_Sub_DONE);
  fCommentAttri_Sub_Warning := TSynHighlighterAttributes.Create(SYNS_AttrComment+'_Warning', SYNS_XML_AttrComment+'_Warning');
  AddAttribute(fCommentAttri_Sub_Warning);
  {/NEW}
  ...
  case fLine[Run] of
    '*':
      begin
        ...
        {/NEW}
        {NEW}
        if (fLine[Run+1]='*') then FSubCommentID:=tsckNote
        else
        if (fLine[Run+1]='-') then FSubCommentID:=tsckDivider
        else
        if (fLine[Run+1]='!') then FSubCommentID:=tsckWarning
        else
        //Todo
        if ((UpperCase(fLine[Run+2])='T') and (UpperCase(fLine[Run+3])='O') and (UpperCase(fLine[Run+4])='D') and (UpperCase(fLine[Run+5])='O')) then
          FSubCommentID:=tsckTODO
        else
        //Done
        if ((UpperCase(fLine[Run+2])='D') and (UpperCase(fLine[Run+3])='O') and (UpperCase(fLine[Run+4])='N') and (UpperCase(fLine[Run+5])='E')) then
          FSubCommentID:=tsckDONE;
        {/NEW}
...
begin
  case GetTokenID of
    ...
    {NEW}
    tkComment: begin
      if (FCommentID=tckAnsi) then begin
        if (FSubCommentID=tsckNote) then Result:=fCommentAttri_Sub_Note
        else
        if (FSubCommentID=tsckDivider) then Result:=fCommentAttri_Sub_Divider
        else
        if (FSubCommentID=tsckTODO) then Result:=fCommentAttri_Sub_TODO
        else
        if (FSubCommentID=tsckDone) then Result:=fCommentAttri_Sub_DONE
        else
        if (FSubCommentID=tsckWarning) then Result:=fCommentAttri_Sub_Warning
        else
        Result:=fCommentAttri_Ansi;
      end
      else
      if (FCommentID=tckBor) then Result:=fCommentAttri_Bor
      else
      if (FCommentID=tckSlash) then Result:=fCommentAttri_Slash
      else
        //Result := fCommentAttri_Sub_Divider
